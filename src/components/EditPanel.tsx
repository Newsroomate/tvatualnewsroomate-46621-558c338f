...
export const EditPanel = ({ isOpen, onClose, item }: EditPanelProps) => {
  ...
  return (
    <div className="fixed top-0 right-0 h-full bg-white border-l border-gray-200 shadow-lg transition-transform duration-300 ease-in-out z-20 min-w-[300px] max-w-[80vw]">
      <ResizablePanelGroup direction="horizontal" className="h-full">
        <ResizablePanel defaultSize={100} minSize={30} className="overflow-y-auto">
        ...
        <ResizableHandle withHandle className="w-2 bg-gray-100 hover:bg-gray-200 cursor-col-resize" />
        
        <ResizablePanel defaultSize={0} minSize={0} maxSize={0} className="hidden">
          {/* Painel oculto para permitir redimensionamento */}
        </ResizablePanel>
      </ResizablePanelGroup>
    </div>
  );
