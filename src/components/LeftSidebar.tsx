...
export const LeftSidebar = ({
  ...
  return <div className="w-64 bg-gray-100 h-full border-r border-gray-200 flex flex-col">
      ...
      <div className="flex-1 overflow-y-auto">
        ...
        <div className="p-4">
          <div className="flex items-center justify-between mb-2">
            ...
            <Button variant="ghost" size="sm" className="h-8 w-8 p-0" onClick={handleOpenTelejornalModal}>
              <PlusCircle className="h-4 w-4" />
              <span className="sr-only">Adicionar Telejornal</span>
            </Button>
          </div>
          {isLoading ? <p className="text-sm text-gray-500">Carregando...</p> : <ul className="space-y-1">
              {telejornais.map(jornal => <li key={jornal.id} className="relative group">
                  <Button variant={selectedJournal === jornal.id ? "secondary" : "ghost"} className={`w-full justify-start text-left pr-16 ${jornal.espelho_aberto ? 'border-l-4 border-green-500 pl-3' : ''}`} onClick={() => handleSelectTelejornal(jornal.id)}>
                    <span className="truncate">{jornal.nome}</span>
                    {jornal.espelho_aberto && <TooltipProvider>
                        <Tooltip>
                  ...
                  <div className="absolute top-1 right-1 hidden group-hover:flex space-x-1">
                    ...
                    <Button variant="ghost" size="icon" className="h-7 w-7 text-red-500 hover:text-red-700" onClick={e => handleDeleteTelejornal(jornal, e)}>
        ...
        <div className="p-4 border-t border-gray-200">
          <div className="flex items-center justify-between mb-2">
            ...
            <Button variant="ghost" size="sm" className="h-8 w-8 p-0" onClick={handleOpenPautaModal}>
              <PlusCircle className="h-4 w-4" />
          ...
          {isLoading ? <p className="text-sm text-gray-500">Carregando...</p> : <ul className="space-y-1">
              {pautas.map(pauta => <li key={pauta.id} className="relative group">
                  ...
                  <div className="absolute top-1 right-1 hidden group-hover:flex space-x-1">
                    ...
                    <Button variant="ghost" size="icon" className="h-7 w-7 text-red-500 hover:text-red-700" onClick={e => handleDeletePauta(pauta, e)}>
                      <Trash2 className="h-4 w-4" />
                      <span className="sr-only">Excluir</span>
                    </Button>
                  </div>
      ...
      {editingTelejornal && <EditTelejornalDialog isOpen={!!editingTelejornal} onClose={() => setEditingTelejornal(null)} telejornal={editingTelejornal} onTelejornalUpdated={loadData} />}
      
      {/* Edit Pauta Dialog */}
      {editingPauta && <EditPautaDialog isOpen={!!editingPauta} onClose={() => setEditingPauta(null)} pauta={editingPauta} onPautaUpdated={loadData} />}
