...
export const ClosedRundownContent = ({
  snapshots,
  isLoading,
  telejornais
}: ClosedRundownContentProps) => {
  ...
  if (selectedSnapshot) {
    return <FullRundownView snapshot={selectedSnapshot} onBack={handleBackToList} />;
  }
  ...
  return <div className="space-y-4">
      ...
      return <SnapshotCard key={snapshot.id} snapshot={snapshot} isExpanded={isExpanded} onToggleExpansion={() => toggleSnapshotExpansion(snapshot.id)} onViewDetails={handleViewDetails} telejornais={telejornais} />;
    })}
    </div>;
};